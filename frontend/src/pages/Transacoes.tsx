...
export function Transacoes() {
  ...
  const syncPluggyTransactions = async () => {
    const selectedMonthDate = startOfMonth(new Date(`${month}-01T12:00:00`));
    const from = format(selectedMonthDate, 'yyyy-MM-dd');
    const to = format(endOfMonth(selectedMonthDate), 'yyyy-MM-dd');

    setIsSyncingPluggy(true);
    try {
      const result = await pluggyApi.syncTransactions(from, to);
      const refreshedTransactions = await transacaoApi.list();
      setTransactions(refreshedTransactions);
      const message = result.imported > 0 || result.removed > 0
        ? `${result.imported} importados e ${result.removed} removidos do Pluggy.`
        : 'Nenhum lançamento novo foi encontrado para este período.';

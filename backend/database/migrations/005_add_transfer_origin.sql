...
ALTER TABLE digfin.transacao
    ADD CONSTRAINT ck_transacao_origem
        CHECK (origem IS NULL OR origem IN ('card', 'pix', 'withdrawal', 'card_refund', 'deposit', 'transfer')),
    ADD CONSTRAINT ck_transacao_origem_recurso
        CHECK (
            ...
            OR (origem IN ('pix', 'withdrawal', 'deposit', 'transfer') AND conta_bancaria_id IS NOT NULL AND cartao_credito_id IS NULL)
